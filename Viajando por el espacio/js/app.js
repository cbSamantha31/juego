...
let punto = document.querySelector("j");
let nivel = document.querySelector("h");
let times = document.getElementById("times");
let lives = 5;
let second = 30;
let velocidadMeteoritos = 10;
...
setInterval(() => {
  ...
  if (second == 0) {
    gameLevel++;
    second = 30;
    velocidadMeteoritos = velocidadMeteoritos + 10;
    if (gameLevel == 4) {
      alert("Felicidades ganaste Los 3 Niveles");
      location.reload();
    } else {
      nivel.textContent = gameLevel;
      alert(`Pasaste al! Nivel ${gameLevel}`);
    }
...
setInterval(() => {
  ...
  enemigos.forEach((element) => {
    element.style.top =
      element.getBoundingClientRect().top + velocidadMeteoritos + "px";
    if (
